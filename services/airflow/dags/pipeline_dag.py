...
models_pipeline_task = BashOperator(
    ...
    bash_command='cd $PYTHONPATH; python3 models/model_pipeline.py',
    dag=dag,
)

deploy_pipeline_task = BashOperator(
    task_id='deploy_pipeline_task',
    bash_command='cd $PYTHONPATH/deployment; echo "ubuntu" | sudo -S sudo timeout 120 docker-compose up && docker-compose down || [[ $? -eq 124 ]]',
    dag=dag,
)
